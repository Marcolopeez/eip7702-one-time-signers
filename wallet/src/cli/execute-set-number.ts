/**
 * Executes a normal setNumber(uint256) operation through the delegated account.
 *
 * This is the happy-path operation flow: sync, derive next auth signer, sign,
 * burn the current auth signer locally, broadcast, wait, then sync again.
 */
import { encodeFunctionData, getAddress, parseEther, type Hex } from "viem";
...
/**
 * Keep value 0 by default.
 *
 * Later we can test payable execution by setting:
 *   OPERATION_VALUE_ETH=0.01
 */
const operationValueEth = optionalEnv("OPERATION_VALUE_ETH", "0");
const operationValue = parseEther(operationValueEth);
...
/**
 * Security rule:
 * Always reconcile before signing.
 */
...
if (
  getAddress(currentAuthSigner.address) !==
  getAddress(state.currentAuthorizedSigner)
) {
  throw new Error(
...
// Select a signer that is not already reserved on-chain before signing it in.
...
// From this point, producing the signature makes currentAuthSigner unsafe to reuse.
 ...
 *
 * From here, auth[currentAuthIndex] is considered burned locally.
 * This happens before broadcasting.
 */
// Persist PENDING_OPERATION before broadcasting, so crashes do not allow key reuse.
...
console.log("Prepared signed setNumber operation");
console.log("-----------------------------------");
...
console.log("");
console.log("Local state saved as PENDING_OPERATION before broadcasting.");
...
// Receipt inclusion is useful, but the semantic result is decided by sync().
...
/**
 * Do not infer final state from receipt.status.
 * Source of truth is contract storage.
 */
...
console.log("");
console.log("Post-execution sync");
console.log("-------------------");
