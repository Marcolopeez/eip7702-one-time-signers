...
import { JsonWalletStateStore } from "../../src/adapters/storage/JsonWalletStateStore.js";
...
describe("JsonWalletStateStore", () => {
  it("saves and loads LocalWalletState", async () => {
    const dir = await mkdtemp(join(tmpdir(), "one-time-signer-wallet-"));
    const path = join(dir, "state.json");

    try {
      const store = new JsonWalletStateStore(path);
      const state = createReadyState();
