...
/// @title OneTimeSignerAccount
/// @notice Minimal EIP-7702 account implementation controlled by rotating one-time ECDSA keys.
...
///      - The delegated EOA is initialized once with its first authorized one-time key, represented
...
contract OneTimeSignerAccount {
    ...
    /// @notice Operation authorized by the current one-time key through its signer address.
    ...
    /// @param nextAuthorizedSigner Signer address for the next one-time key.
    ...
    /// @dev EIP-712 type hash for operations authorized by the current one-time key's signer address.
    ...
    bytes32 public constant NAME_HASH = keccak256("OneTimeSignerAccount");
    ...
    /// @notice Signer address of the current one-time ECDSA key.
    ...
    /// @notice Initializes the delegated EOA with its first one-time key and recovery key signers.
    ...
    /// @param firstAuthorizedSigner Signer address of the first one-time key allowed to authorize this account.
    ...
    /// @notice Rotates the current one-time key when called directly by its authorized signer address.
    ...
    /// @notice Rotates the current one-time key and renews recovery when called directly by a recovery key signer.
    ...
    /// @notice Executes an operation authorized by the current one-time key and rotates to the next key.
    /// @dev The signature is verified first. If it was produced by `currentAuthorizedSigner`, the underlying
    ///      one-time key is consumed immediately by rotating to `operation.nextAuthorizedSigner`.
    ///
    ...
    ///
    /// @param operation Operation authorized by the current one-time key.
    /// @param signature EIP-712 signature over `operation`, produced by the current one-time key.
        ...
        returns (bool success, bytes memory result)
    {
        ...
        // Consume the one-time key before validating the executable part of the operation.
    ...
    /// @notice Computes the EIP-712 digest signed by the current one-time key.
    ...
    ///
    ///      At this point, the signature has already been verified and the one-time key has already
